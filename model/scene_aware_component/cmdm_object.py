...
from model.mdm import MDM
from model.scene_aware_component.cmdm import zero_module, HintBlock, PositionalEncoding, TimestepEmbedder
import math

class CMDM_Object(MDM):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        super(CMDM_Object, self).__init__(modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        # --- ! CMDM ---
        # ! finetune this | we copy the same module, but with an extra scene information.
        
        if self.input_feats <= 5:
        ...
        if self.cond_mode != 'no_cond': # this is extra text condition;
            # ! the finetune branch only add object information.
            if 'BPS' in self.cond_mode:
                ...
                if 'concatefeature2' not in self.cond_mode:
                    if 'objectv2BPS' in self.cond_mode:
                        self.c_merge_bps_info = ObjectEncoder(
                            'bps', # data_rep
                            1000*7, # [bps point xyz, w/o], bps point->object distance, bps point->body joint distance, with bps point->body joint nearest label.
                            self.latent_dim,
                        )
                    else:
                        self.c_merge_bps_info = ObjectEncoder(
    ...
    def mask_cond_no_object(self, cond, y_flag, force_mask=False):
        ...
        elif self.training and self.cond_mask_prob_noObj > 0.: # drop condition
        # elif self.training: # drop condition
            import pdb;pdb.set_trace()
            assert y_flag is not None
            mask = torch.bernoulli(torch.ones(bs, device=cond.device) * self.cond_mask_prob).view(bs, 1)  # 1-> use null_cond, 0-> use real cond
            if y_flag.sum() > y_flag.shape[0] * (1 - self.cond_mask_prob_noObj): # if the dropped object is not sufficient.
                selected_obj_num = math.ceil(y_flag.sum() - y_flag.shape[0] * (1 - self.cond_mask_prob_noObj))
                select_obj_idx = mask.nonzero()[:selected_obj_num].squeeze()
                y_flag[select_obj_idx] = 0
            if len(cond.shape) == 4:
                return cond * (1. * y_flag.view(bs, 1, 1, 1)) # 1: with object; 0 with dropped out object
            elif len(cond.shape) == 3:
                return cond * (1. * y_flag.view(bs, 1, 1))
        else:
            return cond 
    ...
    def cmdm_forward_attnmask(self, x, timesteps, y=None, weight=1.0):
        ...
        # import pdb;pdb.set_trace()
        if self.double_indicator and x.shape[1] > 5:
        ...
        # import pdb;pdb.set_trace()
        # x original shape: bs, dim, 1, frames
        # obj_coord_poses: bs, frames, 22, 3
        frames = nframes
        ...
        bps_input_feat = torch.stack((bps_data[:, None].repeat(1, frames, 1), bps_to_joint_dis, bps_to_joint_idx), axis=-1) # batch, frames, 1000, 3 | version1;
        # bps_input_feat.shape: torch.Size([64, 196, 1000, 3])

        
        x = self.c_input_process(x)
            # x.shape: torch.Size([196, 64, 512])
        ...
        if self.cfg_noObj:
            ...
            # import pdb;pdb.set_trace()
            # zero out the object information to the transformer.
            x = self.mask_cond_no_object(x.permute(1,0,2), obj_flag, force_mask=force_mask).permute(1,0,2)
        ...
        if 'extraToken' in self.cond_mode:
            # import pdb;pdb.set_trace()
            xseq = torch.cat((emb, emb, x), axis=0)  # [seqlen+2, bs, d]
            ...
            output = self.c_seqTransEncoder(xseq, src_key_padding_mask=~aug_mask[:,:,0].permute(1,0))  # , src_key_padding_mask=~maskseq)  # [seqlen, bs, d]
        else:
        ...
        control = control * weight # * seq_mask.permute(1, 0).unsqueeze(0).unsqueeze(-1)
        return control
    ...
    def mdm_forward(self, x, timesteps, y=None, control=None):
        ...
        if self.arch == 'trans_enc':
            ...
            if 'extraToken' in self.cond_mode:
                ...
                xseq = self.sequence_pos_encoder(xseq)  # [seqlen+1, bs, d]
                aug_mask = torch.cat((time_mask, time_mask, x_mask), 0).to(xseq.device) # ignore the attention for the padding elements
                output = self.seqTransEncoder(xseq, src_key_padding_mask=~aug_mask[:,:,0].permute(1,0), control=control)[2:]  # , src_key_padding_mask=~maskseq)  # [seqlen, bs, d]
            else:
                xseq = torch.cat((emb, x), axis=0)  # [seqlen+1, bs, d]
                xseq = self.sequence_pos_encoder(xseq)  # [seqlen+1, bs, d]
                aug_mask = torch.cat((time_mask, x_mask), 0).to(xseq.device) # ignore the attention for the padding elements
                output = self.seqTransEncoder(xseq, src_key_padding_mask=~aug_mask[:,:,0].permute(1,0), control=control)[1:]  # , src_key_padding_mask=~maskseq)  # [seqlen, bs, d]
    ...
    def forward(self, x, timesteps, y=None):
        ...
        # import pdb;pdb.set_trace()
        # --realism guidance--
        # hint: spatial control signal (dimension: BxNx(Jx3), N=196 number of sequence, J=22 number of joints)
        # if 'hint' in y.keys():

        if 'scene' in y.keys(): # 'scene' means for 2D floor masks; 
            # control = self.cmdm_forward_attnmask(x, timesteps, y) # this works for object.
            if x.shape[1] > 5:
