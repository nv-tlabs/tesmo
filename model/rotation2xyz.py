...
class Rotation2xyz:
    def __init__(self, device, dataset='amass', batch_size=None):
        ...
        if dataset in ['babel', 'humanmlabsolute']:
            self.smpl_model = SMPLH(path='./body_models/smpl_models/smplh', jointstype='smplnh', input_pose_rep='matrix', batch_size=batch_size, gender='male', name='SMPLH').eval().to(device)

        else:
            self.smpl_model = SMPL(return_vertices=True).eval().to(device)
            # self.smpl_model = SMPLLayer().eval().to(device)

    def __call__(self, x, mask, pose_rep, translation, glob,
                 jointstype, vertstrans, betas=None, beta=0,
                 glob_rot=None, get_rotations_back=False, data_type=None, **kwargs):
        
        if self.dataset in ['babel', 'humanmlabsolute']:
