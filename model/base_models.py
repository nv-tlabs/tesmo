...
from model.tensor_utils import reshape_dimensions
import model.tensor_utils as TensorUtils
...
class ResNetUNet(nn.Module): # this is too heavy;

    def __init__(self, input_dim, n_class):
        super().__init__()
        
        self.base_model = resnet18(pretrained=False) # old version: pretrained=True
        ...
        self.conv_original_size0 = convrelu(input_dim, 64, 3, 1)
...
class RasterizedMapEncoder(nn.Module):
    ...
    def __init__(
        ...
        # elif model_arch == "resnet18unet":
        #     self.map_model = ResNetUNet(64) # output 64 feature maps;
        #     out_h = int(math.ceil(input_image_shape[1] / 32.))
        #     out_w = int(math.ceil(input_image_shape[2] / 32.))
        #     self.conv_out_shape = (64, out_h, out_w)
            # self.conv_out_shape = (64, )
        else:
            raise NotImplementedError(f"Model arch {model_arch} unknown")

        # configure spatial reduction pooling layer; Below are not used.
