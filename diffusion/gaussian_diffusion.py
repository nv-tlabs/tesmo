...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        lambda_vel_rcxyz=0.,
        lambda_fc=0.,
        lambda_extra_weight=1.0,
        lambda_extra_dim='',
        batch_size=32,
        multi_train_mode=None,
        indicator_gd=False,
        lambda_start_end_weight=0.0,
        lambda_vel_rcxyz_startEnd=0.0,
        indicator=False, # not used.
        cfg_startEnd=False,
        cfg_startEnd_addNoise=False,
        cond_mask_prob_startEnd=0.1,
        mask_out=True, # this is used in the inpainting model.
        no_last_frame_inference=False,
    ):
        ...
        self.lambda_extra_weight = lambda_extra_weight
        if lambda_extra_dim != '':
            self.lambda_extra_dim = [int(tmp) for tmp in lambda_extra_dim.split(',')]
        else:
            self.lambda_extra_dim = []
        ...
        self.indicator_gd =indicator_gd
        
        self.no_last_frame_inference = no_last_frame_inference
        
        self.cond_mask_prob_startEnd = cond_mask_prob_startEnd
        self.cfg_startEnd = cfg_startEnd
        self.cfg_startEnd_addNoise = cfg_startEnd_addNoise

        # self.cond_mask_prob_startEnd = cond_mask_prob_startEnd
        # self.cfg_startEnd = cfg_startEnd
        # self.cfg_startEnd_addNoise = cfg_startEnd_addNoise
        
        # focus more on start and end point.
        self.lambda_start_end_weight = lambda_start_end_weight
        self.lambda_vel_rcxyz_startEnd = lambda_vel_rcxyz_startEnd
    ...
    def masked_l2(self, a, b, mask, weight=None):
    ...
    def q_sample(self, x_start, t, noise=None, model_kwargs=None): # trainning.
        """
    ...
    def p_mean_variance(
        ...
        # import pdb;pdb.set_trace()
        # rewrite the inpainting motion for the input;
        # if 'inpainting_mask' in model_kwargs['y'].keys() and 'inpainted_motion' in model_kwargs['y'].keys():
        #     inpainting_mask, inpainted_motion = model_kwargs['y']['inpainting_mask'], model_kwargs['y']['inpainted_motion']
        #     ones = torch.ones_like(inpainting_mask, dtype=torch.float, device=inpainting_mask.device) # this is the model prediction;
        #     inpainting_mask = ones * inpainting_mask
        #     x = (x * (1 - inpainting_mask)) + (inpainted_motion * inpainting_mask)
        #  
        # TODO: add condition here

        model_output = model(x, self._scale_timesteps(t), **model_kwargs) # ! inference transform model. 
        ...
        if 'inpainting_mask' in model_kwargs['y'].keys() and 'inpainted_motion' in model_kwargs['y'].keys():
            ...
            # import pdb;pdb.set_trace()
            '''
            if 'keep_last_step' in model_kwargs['y'].keys() and model_kwargs['y']['keep_last_step'] and t[0] == 0 : # save last time step prediction results;
                pass
                print('save the last time step prediction results;')
            if 'keep_n_step' in model_kwargs['y'].keys() and t[0] < model_kwargs['y']['keep_n_step']: # save last time step prediction results;
                pass
                print(f"save the prediction results; {t[0]} / {model_kwargs['y']['keep_n_step']}")
            else:
                # TODO: add supervision on this case;
            ...
            ori_model_output = model_output.clone()
            if t[0] == 0 and (('keep_n_step' in model_kwargs['y'].keys() and model_kwargs['y']['keep_n_step'] != 0) or \
                ('keep_last_step' in model_kwargs['y'].keys() and model_kwargs['y']['keep_last_step'])):
                print('keep the last time step prediction results.')
            else:
                # inpainting_mask supports both boolean and scalar values
                ones = torch.ones_like(inpainting_mask, dtype=torch.float, device=inpainting_mask.device) # this is the model prediction;
                inpainting_mask = ones * inpainting_mask
        ...
        # blend the result from A* start and use it for the next    
        #  
        # import pdb;pdb.set_trace()
        if 'extra_sample_motion' in model_kwargs['y'].keys(): 
            if t[0] == 0 and (('keep_n_step' in model_kwargs['y'].keys() and model_kwargs['y']['keep_n_step'] != 0) or \
                ('keep_last_step' in model_kwargs['y'].keys() and model_kwargs['y']['keep_last_step'])):
                # import pdb;pdb.set_trace()
                print('keep the last time step prediction results. ******** !!!!!!')
            else:
                extra_sample_motion = model_kwargs['y']['extra_sample_motion'] # only works on the trajectory xz position.
                blend_scale = model_kwargs['y']['blend_scale']
                
                # import pdb;pdb.set_trace()
                # blend_mask = torch.ones_like(model_output[..., :2], dtype=torch.float, device=model_output.device)
                for tmp_i, tmp_len in enumerate(model_kwargs['y']['lengths']):

                    if 'blend_mask' in model_kwargs['y'].keys(): # this is for motion blend. 
                        blend_mask = model_kwargs['y']['blend_mask'][tmp_i] # only rewrite the very first 10 frames.
                        model_output[tmp_i, :, :, :tmp_len][blend_mask[:, :, :tmp_len].bool()] = (model_output[tmp_i, :, :, :tmp_len] * (1- blend_scale) + extra_sample_motion[tmp_i, :, :, :tmp_len] * blend_scale)[blend_mask[:, :, :tmp_len].bool()]

                    else:
                        # only consider xz for input trajectory of A*.
                        model_output[tmp_i, :2, :, 1:tmp_len] = model_output[tmp_i, :2, :, 1:tmp_len] * (1- blend_scale) + extra_sample_motion[tmp_i, :2, :, 1:tmp_len] * blend_scale

        # ------------------- sample method ------------------

        ### different kinds of diffusion model predictions.
        ...
        # import pdb;pdb.set_trace()
        def process_xstart(x): # currently return x;
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            if self.model_mean_type == ModelMeanType.START_X: # THIS IS MDM
                pred_xstart = process_xstart(model_output) # ! they are equal!
            else:
                pred_xstart = process_xstart( # EPSILON
        ...
        # import pdb;pdb.set_trace()
        ### we keep the remaining output.
        if ori_model_output is not None:
            if self.model_mean_type == ModelMeanType.START_X: # THIS IS MDM
                # save last time step prediction results;
                if 'keep_n_step' in model_kwargs['y'].keys() and t[0] < model_kwargs['y']['keep_n_step'] and t[0] > 0: # this is used for calculate the prediction loss.
                    print(f"save the prediction results; {t[0]} / {model_kwargs['y']['keep_n_step']}") 
                    pred_xstart = process_xstart(ori_model_output) # ! they are equal!
                else:
                    print('the guidance is zero.')
            ...
            return {
                ...
                "pred_xstart_overwrite": model_output,
            }
        else:
            return {
                "mean": model_mean,
    ...
    def condition_mean(self, cond_fn, p_mean_var, x, t, model_kwargs=None, version=1): # get the gradient;
        """
        ...
        """
        # This is epsilon prediction.
        
        # print('run condition_mean')
        # import pdb;pdb.set_trace()
        
        if False:
            gradient = cond_fn(x, self._scale_timesteps(t), p_mean_var=p_mean_var, **model_kwargs)
            import pdb;pdb.set_trace()
            new_mean = (
                p_mean_var["mean"].float() + p_mean_var["variance"] * gradient.float()
            )
        elif version == 1: # this is the first version that I am using before 15th Sep.
            
            # import pdb;pdb.set_trace()
            if type(cond_fn) == list: # multiple conditions
                all_gradient = None
                for conf_fn_sample in cond_fn:
                    gradient = conf_fn_sample(p_mean_var["pred_xstart"], # ! this should be noised input.
                                                self._scale_timesteps(t), 
                                            p_mean_var=p_mean_var,
                                                **model_kwargs)
                    # gradient = conf_fn_sample(p_mean_var["pred_xstart"], self._scale_timesteps(t), p_mean_var=p_mean_var, **model_kwargs)
                    if all_gradient is None:
                        all_gradient = gradient.float()
                    else:
                        all_gradient += gradient.float()
                
                new_mean = (
                    p_mean_var["mean"].float() + all_gradient
                )
                
            else:
                gradient = cond_fn(p_mean_var["pred_xstart"], # ! this should be noised input.
                                    self._scale_timesteps(t), 
                                        p_mean_var=p_mean_var,
                                            **model_kwargs)
                # new_mean = (
    ...
    def condition_mean_with_grad(self, cond_fn, p_mean_var, x, t, model_kwargs=None):
        ...
        # original version.
        # print('run condition_mean_with_grad')
        ...
        if type(cond_fn) == list: # multiple conditions
            gradient = None
            for conf_fn_sample in cond_fn:
                gradient_tmp = conf_fn_sample(x, # ! this should be noised input.
                                        self._scale_timesteps(t), 
                                    p_mean_var=p_mean_var,
                                        **model_kwargs)
                if gradient is None:
            ...
            gradient = cond_fn(x, # ! this should be noised input.
                                            self._scale_timesteps(t), 
                                        p_mean_var=p_mean_var,
                                            **model_kwargs)
            
        updated_xstart = p_mean_var["pred_xstart"] + gradient.float() # p_mean_var["pred_xstart"] should be the one from overwrite one results.
            
        # indicator should now change.   
        # updated_xstart = p_mean_var["pred_xstart"] + p_mean_var["variance"] * gradient.float()
    ...
    def p_sample(
        ...
        ### ! inference guidance to change the out['mean'].
        if cond_fn is not None: # update the new mean;
        ...
        # this is for the next step input.
        sample = out["mean"] + nonzero_mask * th.exp(0.5 * out["log_variance"]) * noise
        
        # update the sample motion;
    ...
    def p_sample_with_grad(
        ...
        """
        with th.enable_grad():
            x = x.detach().requires_grad_() # noised input

            # for n, v in model.model.named_parameters(): print(n, v._version, v.requires_grad)
            out = self.p_mean_variance(
                model,
                x,
                t,
            ...
            noise = th.randn_like(x) # add extra noise.
            # print('const_noise', const_noise)
            if const_noise:
                noise = noise[[0]].repeat(x.shape[0], 1, 1, 1)
            nonzero_mask = (
            ...
            # if cond_fn is not None and t[0] != 0: # not update the gradient at the last step
            if self.no_last_frame_inference:
                if cond_fn is not None and t[0] != 0:
                    out["mean"] = self.condition_mean_with_grad( # with more steps;
                    cond_fn, out, x, t, model_kwargs=model_kwargs
                )
            else:
        ...
        # TODO: check whether pred_xstart needs to detach or not.
        return {"sample": sample, "pred_xstart": out["pred_xstart"].detach()}

    def p_sample_loop( # This is used.
        self,
        model,
        shape,
        noise=None,
    ...
    def p_sample_loop_progressive(
        ...
        # TODO: sample twice for one adding noise model, and another without adding noise model.
        ...
        if noise is not None:
            ...
            if predict_two_person:
                ...
                img = th.randn(*shape, device=device) # this is US
        ...
        # used for initialization. not run.
        ...
        for i in indices:
            ...
            with th.no_grad(): # this is old version.
            # with th.enable_grad():
                # import pdb;pdb.set_trace()
                # cond_fn_with_grad is False
                ...
                out = sample_fn(
                    model,
                    img, # the sample result will be the next step input.
                    t,
                    clip_denoised=clip_denoised,
                    denoised_fn=denoised_fn,
                    cond_fn=cond_fn,
                ...
                yield out
                img = out["sample"] # last output sample will be the next input.
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        :param x_start: the [N x C x ...] tensor of inputs. | final predict results.
        ...
        enc = model.model
        mask = model_kwargs['y']['mask'] # mask out those not used variables.
        ...
        # x_start -> diffusion process to x_t
        x_t = self.q_sample(x_start, t, noise=noise, model_kwargs=model_kwargs) # is used for initialization. and training; 
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
        ...
        elif self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE: # this is US.

            # !! this is for transformer encoder.
            ...
            # TODO: add extra loss;
            end_lengths = model_kwargs['y']['lengths']
            
            if self.lambda_extra_weight != 1.0 and len(self.lambda_extra_dim) > 0:
                extra_weight = mask.repeat((1, target.shape[1], 1, 1)).float()
                if self.indicator_gd: # always be the last one.
                    extra_weight[:, -1, :, :] *= 0.0
                extra_weight[:, self.lambda_extra_dim, :, :] *= self.lambda_extra_weight
                
                terms["rot_mse"] = self.masked_l2(target, model_output, mask, weight=extra_weight) # mean_flat(rot_mse)
            # elif self.lambda_start_end_weight != 1.0:
            #     extra_weight = mask.repeat((1, target.shape[1], 1, 1)).float()
            #     for tmp_i in range(end_lengths.shape[0]):
            #         tmp_v = end_lengths[tmp_i]
            #         extra_weight[:, :, :, tmp_v-1] *= self.lambda_start_end_weight
            #         extra_weight[:, :, :, 0] *= self.lambda_start_end_weight
            #     terms["rot_mse"] = self.masked_l2(target, model_output, mask, weight=extra_weight) # mean_flat(rot_mse)
            elif self.indicator_gd:
                extra_weight = mask.repeat((1, target.shape[1], 1, 1)).float()
                if self.indicator_gd: # always be the last one.
                    extra_weight[:, -1, :, :] *= 0.0
                terms["rot_mse"] = self.masked_l2(target, model_output, mask, weight=extra_weight)
                
            else:
                terms["rot_mse"] = self.masked_l2(target, model_output, mask) # mean_flat(rot_mse)
            
            # original start and end point prediction.
            
            # for extra supersion.
            # bs, feat, 1, frames
            root_dim = [0,1,2,3,4]
            terms["pos_start"] = self.l2_loss(target[:, root_dim, :, 0], model_output[:, root_dim, :, 0]).mean(1).squeeze() # mean over every element.
            pos_end_loss = []
            for tmp_i in range(end_lengths.shape[0]): 
                tmp_v = end_lengths[tmp_i]
                pos_end_loss.append(self.l2_loss(target[tmp_i, root_dim, :, tmp_v-1], model_output[tmp_i, root_dim, :, tmp_v-1]).mean())
            terms["pos_end"] =  torch.stack(pos_end_loss)
            
            
            ### TODO: add extra metrics: velocity on xyz, velocity on the rotation, delta along with the trajectory;
            
            import pdb;pdb.set_trace()
            if self.lambda_vel_rcxyz_startEnd > 0.:
                if target.shape[1] < 6:
                    ele_idx = [0,1]
                else:
                    ele_idx = [0,1,4]
                target_vel_rcxyz_sE = target[:, ele_idx, :, 1:] - target[:, ele_idx, :, :-1]
                pred_vel_rcxyz_sE = model_output[:, ele_idx, :, 1:] - model_output[:, ele_idx, :, :-1]
                terms['vel_rcxyz_startEnd'] = self.masked_l2(target_vel_rcxyz_sE, pred_vel_rcxyz_sE, mask[..., 1:])   
                
            
            target_xyz, model_output_xyz = None, None

            if False and (self.lambda_rcxyz or self.lambda_vel_rcxyz or self.lambda_fc or self.lambda_vel):
            ...
            if self.lambda_rcxyz > 0.:
                ...
                terms["rcxyz_mse"] = self.masked_l2(target[:, :5, :, :], model_output[:, :5, :, :], mask)

                # terms["rcxyz_mse"] = self.masked_l2(target_xyz, model_output_xyz, mask)  # mean_flat((target_xyz - model_output_xyz) ** 2)

            # add extra local velocity x,y,z losses; | orientation seems ok.
            import pdb;pdb.set_trace()
            if self.lambda_vel_rcxyz > 0.:
                if self.data_rep == 'rot6d': # and dataset.dataname in ['humanact12', 'uestc', 'BABEL', 'babel', 'humanmlabsolute']:
                    ...
                    target_xyz_vel = (target[:, :5, :, 1:] - target[:, :5, :, :-1])
                    model_output_xyz_vel = (model_output[:, :5, :, 1:] - model_output[:, :5, :, :-1])
                    terms["vel_xyz_mse"] = self.masked_l2(target[:, :5, :, :], model_output[:, :5, :, :], mask)
            ...
            terms["loss"] = terms["rot_mse"] + terms.get('vb', 0.) +\
                            ...
                            (self.lambda_start_end_weight * (terms.get('pos_end', 0.))) +\
                            (self.lambda_vel_rcxyz_startEnd * terms.get('vel_rcxyz_startEnd', 0.))

            # terms.get('pos_start', 0.) +
