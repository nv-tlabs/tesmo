...
class MeshViewer(object):
    ...
    def add_mesh_seq(self, mesh_seq, color_name=None):
        ''' 
        ...
        for mid, mesh in enumerate(mesh_seq):
            ...
            if isinstance(mesh, trimesh.Trimesh):
                ...
                if color_name is None:
                    ...
                    tmp_mesh = mesh.copy()
                    face_colors = np.array([colors[color_name]]*len(tmp_mesh.faces))
                    # tmp_mesh.visual.vertex_colors = face_colors
                    tmp_mesh = trimesh.Trimesh(vertices=tmp_mesh.vertices, faces=tmp_mesh.faces, face_colors=face_colors, process=False)
                    mesh = pyrender.Mesh.from_trimesh(tmp_mesh, smooth=False)
                    # mesh = pyrender.Mesh.from_trimesh(mesh.copy(), face_color=np.array([colors[color_name]]*len(mesh.faces)))
