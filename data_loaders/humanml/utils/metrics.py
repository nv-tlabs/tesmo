...
# evaluate the skating ratio.
def calculate_skating_ratio(motions, all_lengths=None, zero_height=False): 
    ...
    if all_lengths is None:
        ...
        for i in range(motions.shape[0]):
            skating_ratio_i, skate_vel_i = calculate_skating_ratio(motions[i:i+1, :, :, :all_lengths[i]], zero_height=zero_height)
            skating_ratio.append(skating_ratio_i)
            skate_vel.append(skate_vel_i)

        skating_ratio_mean = np.array(skating_ratio).mean()
        return skating_ratio_mean, np.array(skating_ratio)
        # skate_vel = np.array(skate_vel).mean()
